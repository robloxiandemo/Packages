...
--[=[
	...
	A fork of Quenty's Maid class, specifically designed for usage with Madwork-related tasks.
]=]
local MadworkMaid = {
...
function Maid:AddCleanupTask(task)
	if self._is_cleaned == true then
		...
		return function() end
	elseif type(task) == "function" or typeof(task) == "RBXScriptConnection" or
		typeof(task) == "Instance" then
